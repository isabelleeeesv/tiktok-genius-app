...
const GeneratorTool = ({ auth, user, db, userData, navigate, guestGenerations, setGuestGenerations, setShowLoginModal }) => {
    ...
    const handleGenerateIdeas = async () => {
        ...
        if (!isSubscribed) {
            if (!user) {
                // Guest: check and increment guestGenerations
                if (remainingGenerations <= 0) {
                    setShowLoginBonusModal(true);
                    return;
                }
                // Increment guest generation count in localStorage
                const today = new Date().toISOString().split('T')[0];
                let stored = JSON.parse(localStorage.getItem('guestGenerations') || '{}');
                if (stored.lastReset !== today) {
                    stored = { count: 0, lastReset: today };
                }
                stored.count = (stored.count || 0) + 1;
                setGuestGenerations(stored);
                localStorage.setItem('guestGenerations', JSON.stringify(stored));
            } else {
